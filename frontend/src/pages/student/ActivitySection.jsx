...
const ActivitySection = () => {
  ...
  const matchedCategory = Array.isArray(categories)
    ? categories.find(
        (cat) => cat.title.toLowerCase() === section.toLowerCase()
      )
    : null;

  const categoryId = matchedCategory?.id;

  const { data: activities } = useActivitiesByCategory(categoryId);
  ...
  const handleActivityResponse = (activityId, response) => {
    if (!user?.id) return;
    submitResponse.mutate({ activityId, responseType: response });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          >
            <ArrowLeft
              className={`h-5 w-5 ${isRTL ? "rotate-180 ml-2" : "mr-2"}`}
            />
            {t("backToDashboard")}
          </Link>

          <div className="flex items-center mb-4">
            <span className="text-4xl mr-4 rtl:ml-4 rtl:mr-0">
              {getSectionEmoji()}
            </span>
            <div>
              ...
              <p className="text-gray-600">
                {t("discoverParticipate")} {section} {t("activities")}
              </p>
            </div>
          </div>
        </div>
