...
const ActivityCard = ({ activity, onResponse, userResponse }) => {
  ...
  const getResponseText = (response) => {
    switch(response) {
      case 'attend': return t('willAttend')
      case 'not_attend': return t('willNotAttend')
      case 'time_conflict': return t('timeNotConvenient')
      default: return ''
    }
  }
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-6">
          ...
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span>{activity.attendeeCount} {t('attending')}</span>
          </div>
          <div className="flex items-center text-sm text-gray-600">
        ...
        {userResponse && (
          <div className="mb-4 p-3 rounded-lg bg-gray-50">
            <p className="text-sm font-medium text-gray-700">
              {t('yourResponse')}: <span className={`${
                userResponse === 'attend' ? 'text-green-600' :
                userResponse === 'not_attend' ? 'text-red-600' :
                'text-amber-600'
              }`}>
